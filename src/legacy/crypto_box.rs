...
//! # #[tokio::main]
//! # async fn main() {
//! use sodoken::legacy::crypto_box::curve25519xsalsa20poly1305::*;
//!
//! // recipient has a keypair
//! let pk = sodoken::legacy::BufWriteSized::new_no_lock();
//! let sk = sodoken::legacy::BufWriteSized::new_mem_locked().unwrap();
...
//! let cipher = sodoken::legacy::BufWrite::new_no_lock(msg.len() + SEALBYTES);
//! seal(cipher.clone(), msg, pk.clone()).await.unwrap();
//! let cipher = cipher.to_read();
//!
//! // recipient can decrypt the message
//! let msg = sodoken::legacy::BufWrite::new_no_lock(cipher.len() - SEALBYTES);
...
//!
//! // recipient has a keypair
//! let r_pk = sodoken::legacy::BufWriteSized::new_no_lock();
//! let r_sk = sodoken::legacy::BufWriteSized::new_mem_locked().unwrap();
//! keypair(r_pk.clone(), r_sk.clone()).await.unwrap();
//! let r_pk = r_pk.to_read_sized();
//! let r_sk = r_sk.to_read_sized();
//!
//! // sender has a keypair
//! let s_pk = sodoken::legacy::BufWriteSized::new_no_lock();
//! let s_sk = sodoken::legacy::BufWriteSized::new_mem_locked().unwrap();
