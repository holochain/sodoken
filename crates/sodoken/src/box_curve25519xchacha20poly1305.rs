...
pub async fn box_extra_easy<M, P, S>(
    message: M,
    dest_pub_key: P,
    src_sec_key: S,
...
pub fn box_open_extra_easy_msg_len(
    cipher_len: usize,
) -> usize {
    cipher_len
...
pub async fn box_open_extra_easy<M, C, P, S>(
    message: M,
    cipher: C,
    src_pub_key: P,
    dest_sec_key: S,
...
mod tests {
    ...
    async fn test_box() -> SodokenResult<()> {
        let src_pub_ = BufWrite::new_no_lock(box_curve25519xchacha20poly1305::BOX_PUBLICKEYBYTES);
        let src_sec = BufWrite::new_no_lock(box_curve25519xchacha20poly1305::BOX_SECRETKEYBYTES);
        let dest_pub_ = BufWrite::new_no_lock(box_curve25519xchacha20poly1305::BOX_PUBLICKEYBYTES);
        let dest_sec = BufWrite::new_no_lock(box_curve25519xchacha20poly1305::BOX_SECRETKEYBYTES);
        let msg = BufRead::new_no_lock(b"test message");

        box_curve25519xchacha20poly1305::box_keypair(src_pub_.clone(), src_sec.clone()).await?;
        box_curve25519xchacha20poly1305::box_keypair(dest_pub_.clone(), dest_sec.clone()).await?;

        let cipher = box_curve25519xchacha20poly1305::box_extra_easy(msg.clone(), dest_pub_.clone(), src_sec.clone()).await?;
        assert_ne!(&*msg.read_lock(), &*cipher.read_lock());

        let msg_len = box_curve25519xchacha20poly1305::box_open_extra_easy_msg_len(cipher.read_lock().len());
        let msg2 = BufWrite::new_no_lock(msg_len);

        box_curve25519xchacha20poly1305::box_open_extra_easy(msg2.clone(), cipher.clone(), src_pub_.clone(), dest_sec.clone()).await?;
    ...
    async fn test_box_seed() -> SodokenResult<()> {
        let src_seed = BufRead::new_no_lock(&[0xdb; box_curve25519xchacha20poly1305::BOX_SEEDBYTES]);
        let src_pub_ = BufWrite::new_no_lock(box_curve25519xchacha20poly1305::BOX_PUBLICKEYBYTES);
        let src_sec = BufWrite::new_no_lock(box_curve25519xchacha20poly1305::BOX_SECRETKEYBYTES);
        let dest_seed = BufRead::new_no_lock(&[0xbd; box_curve25519xchacha20poly1305::BOX_SEEDBYTES]);
        let dest_pub_ = BufWrite::new_no_lock(box_curve25519xchacha20poly1305::BOX_PUBLICKEYBYTES);
        let dest_sec = BufWrite::new_no_lock(box_curve25519xchacha20poly1305::BOX_SECRETKEYBYTES);
        let msg = BufRead::new_no_lock(b"test message");

        box_curve25519xchacha20poly1305::box_seed_keypair(src_pub_.clone(), src_sec.clone(), src_seed.clone()).await?;
        box_curve25519xchacha20poly1305::box_seed_keypair(dest_pub_.clone(), dest_sec.clone(), dest_seed.clone()).await?;
